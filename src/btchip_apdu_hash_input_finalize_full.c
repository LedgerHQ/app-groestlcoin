  ...
  if (btchip_context_D.tmpCtx.output.changeInitialized && !isOpReturn) {
    ...
    if (!isP2sh &&
        os_memcmp(btchip_context_D.currentOutput + 8 + addressOffset,
                  btchip_context_D.tmpCtx.output.changeAddress + 1, 20) == 0) {
      changeFound = true;
  ...
  BEGIN_TRY {
    TRY {
      ...
      if (p1 == FINALIZE_P1_CHANGEINFO) {
        ...
        os_memset(transactionSummary, 0, sizeof(btchip_transaction_summary_t));
        ...
        btchip_private_derive_keypair(transactionSummary->summarydata.keyPath,
                                      1, NULL);
        if (((N_btchip.bkp.config.options & BTCHIP_OPTION_UNCOMPRESSED_KEYS) !=
             0)) {
          keyLength = 65;
        } else {
        ...
        os_memmove(btchip_context_D.tmpCtx.output.changeAddress,
        ...
        // unusual, ask for user approval
        if (bip44_derivation_guard(transactionSummary->summarydata.keyPath,
                                   true)) {
          btchip_context_D.io_flags |= IO_ASYNCH_REPLY;
          btchip_context_D.outputParsingState =
              BTCHIP_BIP44_CHANGE_PATH_VALIDATION;
          btchip_bagl_request_change_path_approval(
              transactionSummary->summarydata.keyPath);
        }
      ...
      if (btchip_context_D.usingSegwit) {
        if (!btchip_context_D.segwitParsedOnce) {
          ...
          cx_hash(&btchip_context_D.transactionHashAuthorization.header,
                  CX_LAST, (unsigned char *)&btchip_context_D.segwit.cache,
                  sizeof(btchip_context_D.segwit.cache), authorizationHash, 32);
        }
      }
      ...
      *target = 0x00;
      ...
      // synchronized)
      if (btchip_context_D.transactionContext.firstSigned) {
        os_memmove(transactionSummary->authorizationHash, authorizationHash,
...
unsigned short btchip_apdu_hash_input_finalize_full() {
  ...
  if (btchip_context_D.io_flags & IO_ASYNCH_REPLY) {
    ...
    if (btchip_context_D.outputParsingState ==
        BTCHIP_BIP44_CHANGE_PATH_VALIDATION) {
      btchip_context_D.outputParsingState =
          BTCHIP_OUTPUT_PARSING_NUMBER_OUTPUTS;
      return sw;
