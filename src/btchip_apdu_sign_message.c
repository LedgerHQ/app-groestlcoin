...
unsigned short btchip_apdu_sign_message_internal() {
  ...
  BEGIN_TRY {
    TRY {
      if (p1 == P1_PREPARE) {
        if ((p2 == P2_FIRST) || (p2 == P2_LEGACY)) {
          ...
          cx_hash(&btchip_context_D.transactionHashFull.header, 0, &chunkLength,
                  1, NULL, 32);
          cx_hash(&btchip_context_D.transactionHashFull.header, 0,
                  btchip_context_D.coinId, btchip_context_D.coinIdLength, NULL,
                  32);
          cx_hash(&btchip_context_D.transactionHashFull.header, 0,
                  (unsigned char *)SIGNMAGIC, SIGNMAGIC_LENGTH, NULL, 32);
          if (btchip_context_D.transactionSummary.messageLength < 0xfd) {
            messageLength[0] =
                btchip_context_D.transactionSummary.messageLength;
            messageLengthSize = 1;
          } else {
            ...
            messageLength[1] =
                (btchip_context_D.transactionSummary.messageLength & 0xff);
            messageLength[2] =
                ((btchip_context_D.transactionSummary.messageLength >> 8) &
                 0xff);
            messageLengthSize = 3;
          }
          cx_hash(&btchip_context_D.transactionHashFull.header, 0,
    ...
    CATCH_ALL { sw = SW_TECHNICAL_DETAILS(0x0F); }
  discard : {
    os_memset(&btchip_context_D.transactionSummary, 0,
              sizeof(btchip_transaction_summary_t));
  }
...
unsigned short btchip_compute_hash() {
  ...
  BEGIN_TRY {
    TRY {
      cx_hash(&btchip_context_D.transactionHashFull.header, CX_LAST, NULL, 0,
              hash, 32);
      btchip_private_derive_keypair(
          btchip_context_D.transactionSummary.summarydata.keyPath, 0, NULL);
