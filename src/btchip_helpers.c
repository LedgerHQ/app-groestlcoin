...
unsigned short
btchip_public_key_to_encoded_base58(unsigned char *in, unsigned short inlen,
...
unsigned char bip44_derivation_guard(unsigned char *bip32Path,
                                     bool is_change_path) {
  ...
  // If the path length is not compliant with BIP44 or if the purpose/coin type
  // don't match regular usage
  ...
  // return a warning
  if ((bip32PathInt[BIP44_ACCOUNT_OFFSET] ^ 0x80000000) >
                  MAX_BIP44_ACCOUNT_RECOMMENDED ||
              bip32PathInt[BIP44_CHANGE_OFFSET] != is_change_path
          ? 1
          : 0 || bip32PathInt[BIP44_ADDRESS_INDEX_OFFSET] >
...
// On the Ledger Blue, if the string is longer than 30 char, the string will be
// split in multiple lines
unsigned char bip32_print_path(unsigned char *bip32Path, char *out,
  ...
  // if the path is longer than 30 char, split the string in multiple strings of
  // length 30
...
void btchip_signverify_finalhash(void *keyContext, unsigned char sign,
  ...
  if (sign) {
    ...
    cx_ecdsa_sign((cx_ecfp_private_key_t *)keyContext,
                  CX_LAST | (rfc6979 ? CX_RND_RFC6979 : CX_RND_TRNG), CX_SHA256,
                  in, inlen, out, outlen, &info);
    if (info & CX_ECCINFO_PARITY_ODD) {
      out[0] |= 0x01;
    }
  } else {
    cx_ecdsa_verify((cx_ecfp_public_key_t *)keyContext, CX_LAST, CX_SHA256, in,
                    inlen, out, outlen);
  }
}
