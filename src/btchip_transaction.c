...
void transaction_offset(unsigned char value) {
  if ((btchip_context_D.transactionHashOption & TRANSACTION_HASH_FULL) != 0) {
    ...
    cx_hash(&btchip_context_D.transactionHashFull.header, 0,
            btchip_context_D.transactionBufferPointer, value, NULL, 32);
  }
...
unsigned long int transaction_get_varint(void) {
  ...
  if (firstByte < 0xFD) {
    ...
    result =
        ...
        ((unsigned long int)(*(btchip_context_D.transactionBufferPointer + 1))
         << 8);
...
void transaction_parse(unsigned char parseMode) {
  ...
  BEGIN_TRY {
    TRY {
      for (;;) {
        ...
        case BTCHIP_TRANSACTION_NONE: {
          ...
          os_memset(
              btchip_context_D.transactionContext.transactionAmount, 0,
          ...
          if (btchip_context_D.usingSegwit) {
            ...
            if (!btchip_context_D.segwitParsedOnce) {
              cx_sha256_init(&btchip_context_D.segwit.hash.hashPrevouts);
              cx_sha256_init(&btchip_context_D.segwit.hash.hashSequence);
            } else {
              ...
              PRINTF("SEGWIT HashedSequence\n%.*H\n",
                     sizeof(btchip_context_D.segwit.cache.hashedSequence),
                     btchip_context_D.segwit.cache.hashedSequence);
              cx_hash(&btchip_context_D.transactionHashFull.header, 0,
                      btchip_context_D.transactionVersion,
                      sizeof(btchip_context_D.transactionVersion), NULL, 32);
              cx_hash(&btchip_context_D.transactionHashFull.header, 0,
                      btchip_context_D.segwit.cache.hashedPrevouts,
                      sizeof(btchip_context_D.segwit.cache.hashedPrevouts),
                      NULL, 32);
              ...
              cx_hash(&btchip_context_D.transactionHashAuthorization.header, 0,
                      ...
                      sizeof(btchip_context_D.segwit.cache), NULL, 32);
            }
          }
          ...
          // Number of inputs
          btchip_context_D.transactionContext
              .transactionRemainingInputsOutputs = transaction_get_varint();
          ...
          if (parseMode == PARSE_MODE_SIGNATURE) {
            ...
            */
            if (btchip_context_D.usingSegwit) {
              ...
              check_transaction_available(36); // prevout : 32 hash + 4 index
              if (!btchip_context_D.segwitParsedOnce) {
                cx_hash(&btchip_context_D.segwit.hash.hashPrevouts.header, 0,
                        btchip_context_D.transactionBufferPointer, 36, NULL, 0);
                transaction_offset_increase(36);
                check_transaction_available(8); // update amount
                btchip_swap_bytes(amount,
                                  btchip_context_D.transactionBufferPointer, 8);
                ...
                os_memmove(btchip_context_D.inputValue,
                           btchip_context_D.transactionBufferPointer, 8);
                transaction_offset_increase(8);
                btchip_context_D.transactionHashOption = TRANSACTION_HASH_FULL;
              }
        ...
        case BTCHIP_TRANSACTION_INPUT_HASHING_IN_PROGRESS_INPUT_SCRIPT: {
          ...
          if (btchip_context_D.transactionContext.scriptRemaining == 0) {
            if (parseMode == PARSE_MODE_SIGNATURE) {
              if (!btchip_context_D.usingSegwit) {
                ...
                if (btchip_context_D.segwitParsedOnce) {
                  ...
                  cx_hash(&btchip_context_D.transactionHashFull.header, 0,
                          btchip_context_D.inputValue, 8, NULL, 0);
                }
              }
            }
            // Sequence
            ...
            if (btchip_context_D.usingSegwit &&
              ...
              cx_hash(&btchip_context_D.segwit.hash.hashSequence.header, 0,
                      btchip_context_D.transactionBufferPointer, 4, NULL, 0);
            }
            transaction_offset_increase(4);
            // Move to next input
            btchip_context_D.transactionContext
        ...
        case BTCHIP_TRANSACTION_INPUT_HASHING_DONE: {
          ...
          if (parseMode == PARSE_MODE_SIGNATURE) {
            ...
            if (btchip_context_D.usingSegwit &&
              ...
              // Flush the cache
              cx_hash(&btchip_context_D.segwit.hash.hashPrevouts.header,
                      CX_LAST, hashedPrevouts, 0, hashedPrevouts, 32);
              cx_hash(&btchip_context_D.segwit.hash.hashSequence.header,
                      CX_LAST, hashedSequence, 0, hashedSequence, 32);
              os_memmove(btchip_context_D.segwit.cache.hashedPrevouts,
                         hashedPrevouts, sizeof(hashedPrevouts));
              os_memmove(btchip_context_D.segwit.cache.hashedSequence,
            ...
            if (btchip_context_D.usingSegwit &&
              ...
              cx_hash(&btchip_context_D.transactionHashFull.header, 0,
                      btchip_context_D.segwit.cache.hashedOutputs,
                      sizeof(btchip_context_D.segwit.cache.hashedOutputs), NULL,
                      0);
              btchip_context_D.transactionContext.transactionState =
    ...
    FINALLY { btchip_set_check_internal_structure_integrity(1); }
  }
  END_TRY;
}
