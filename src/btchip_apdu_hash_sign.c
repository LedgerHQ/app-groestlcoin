...
unsigned short btchip_apdu_hash_sign() {
  ...
  BEGIN_TRY {
    TRY {
      ...
      if (((N_btchip.bkp.config.options & BTCHIP_OPTION_FREE_SIGHASHTYPE) ==
           0)) {
...
#define SIGHASH_FORKID 0x40
      ...
      // TODO : remove copy
      os_memmove(&transactionSummary, &btchip_context_D.transactionSummary,
      ...
      cx_hash(&btchip_context_D.transactionHashFull.header, CX_LAST, dataBuffer,
              sizeof(dataBuffer), hash, 32);
      PRINTF("Hash1\n%.*H\n", sizeof(hash), hash);

      // Sign
      btchip_signverify_finalhash(
          &btchip_private_key_D, 1, hash, sizeof(hash), G_io_apdu_buffer,
