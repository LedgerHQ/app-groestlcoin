...
unsigned short btchip_apdu_get_wallet_public_key() {
  ...
  if (display_request_token) {
    ...
    request_token =
        btchip_read_u32(G_io_apdu_buffer + request_token_offset, true, false);
  }
    ...
    if (!nativeSegwit) {
      ...
      if (G_coin_config->native_segwit_prefix) {
        keyLength = segwit_addr_encode((char *)(G_io_apdu_buffer + 67),
                                       PIC(G_coin_config->native_segwit_prefix),
                                       0, tmp + 2, 20);
        if (keyLength == 1) {
  ...
  // If the token requested has already been approved in a previous call, the
  // source is trusted so don't ask for approval again
  else if (display_request_token &&
           (!btchip_context_D.has_valid_token ||
